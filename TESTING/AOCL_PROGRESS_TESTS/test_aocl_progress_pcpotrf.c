...
void pcpotrf_(char*, Int*, SL_complex_float*, Int*, Int*, Int*, Int*);
...
int main(int argc, char **argv) {
    ...
    // Run pcpotrf and measure time
    double MPIt1 = MPI_Wtime();
    printf("[%dx%d] Starting pcpotrf\n", myrow, mycol);
    aocl_scalapack_set_progress(&AOCL_progress);
    pcpotrf_(&uplo, &n, A, &ione, &ione, descA, &info);
    if (info != 0) {
        printf("Error in pcpotrf, info = %d\n", info);
    }
