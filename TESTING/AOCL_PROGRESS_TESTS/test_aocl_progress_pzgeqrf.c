...
void pzgeqrf_(Int*, Int*, SL_complex_double*, Int*, Int*, Int*, SL_complex_double*, SL_complex_double*, Int*, Int*);
...
int main(int argc, char **argv) {
    ...
    printf("Usage: ./test matrix_size_rows matrix_size_columns block_size nprocs_row nprocs_col\n");
    ...
    if(argc > 5) {
        nprow = (Int)atoi(argv[5]);
    }
    if(argc > 6) {
        npcol = (Int)atoi(argv[6]);
    }
    ...
    SL_complex_double work_buffer_size;
    SL_complex_double *work, *tau;
    Int lwork = -1;
    tau = (SL_complex_double *)calloc((mpA+nqA),sizeof(SL_complex_double)) ;
    ...
    pzgeqrf_(&m, &n, A, &ione, &jone, descA, tau, &work_buffer_size, &lwork, &info);

    work = (SL_complex_double *)calloc(work_buffer_size, sizeof(SL_complex_double)) ;
    lwork = work_buffer_size;

    // Run pzgeqrf_ and time
    float MPIt1 = MPI_Wtime();
    printf("[%dx%d] Starting pzgeqrf\n", myrow, mycol);
    aocl_scalapack_set_progress(&AOCL_progress);
    pzgeqrf_(&m, &n, A, &ione, &jone, descA, tau, work, &lwork, &info);
    if (info != 0) {
        printf("Error in pzgeqrf, info = %i\n", info);
    }
